...
public class login extends database implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == register) {
			// Must implement register.java file (coming soon)
			// new Register();
			return;
		}
		String user = typeUserName.getText();
		String pass = typePassword.getText();// getText() is deprecated
		if (e.getSource() == login) {
			if (database.users.containsKey(user) && database.users.get(user).equals(pass)) {
				success.setText("Login successful!");
				success.setForeground(Color.GREEN);
				frame.dispose();
				new vehicularCloudConsole();
			} else {
				success.setText("Username or password is incorrect! Try again!");
				clearFields();
			}
		}
